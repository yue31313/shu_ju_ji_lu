...
public class WheelView extends View {
	...
	private void initResourcesIfNecessary() {
		...
		double val = TEXT_SIZE * density;
		
		if (itemsPaint == null) {
			itemsPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
			//itemsPaint.density = getResources().getDisplayMetrics().density;
			
			itemsPaint.setTextSize((int)val);
		...
		if (valuePaint == null) {
			...
			valuePaint.setTextSize((int)val);
	...
	private int getDesiredHeight(Layout layout) {
		...
		dm = this.getResources().getDisplayMetrics();
		double density = dm.density;
		double val1 = ITEM_OFFSET * density;
		
		double val2 = ADDITIONAL_ITEM_HEIGHT * density;
		int desired = (int) (getItemHeight() * visibleItems - val1 * 2
				- val2);
...
//				- ADDITIONAL_ITEM_HEIGHT;
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		dm = this.getResources().getDisplayMetrics();
		double density = dm.density;
		double val1 = ADDITIONAL_ITEMS_SPACE * density;
		...
		if (mode == MeasureSpec.EXACTLY) {
			...
			width = (int) (itemsWidth + labelWidth + 2 * PADDING * density);
			if (labelWidth > 0) {
				width += LABEL_OFFSET* density;
			}
		...
		if (recalculate) {
			// recalculate width
			int pureWidth = (int) (width - LABEL_OFFSET* density - 2 * PADDING * density);
			if (pureWidth <= 0) {
			...
			if (labelWidth > 0) {
				...
				itemsWidth = (int) (pureWidth + LABEL_OFFSET* density); // no label
			}
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (itemsWidth > 0) {
			...
			canvas.translate((float) (PADDING * density), -ITEM_OFFSET);
			drawItems(canvas);
			drawValue(canvas);
			canvas.restore();
		}
	...
	private void drawValue(Canvas canvas) {
		...
		if (labelLayout != null) {
			canvas.save();
			canvas.translate((float) (itemsLayout.getWidth() + LABEL_OFFSET* density), bounds.top);
